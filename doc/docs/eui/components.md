...
| Signature | Notes |
|---|---|
| `chart_line(values, w, h)` | Grid, polyline, a dot per point |
| `chart_area(values, w, h)` | Grid, filled area, line on top |
| `chart_bar(values, w, h)` | Grid and bars, each an eighth of a slot apart |
| `chart_donut(parts, w, h)` | One arc per part, in the four `base` roles |
