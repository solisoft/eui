    ...
    fn encode(&self) -> Vec<u8> {
        ...
        match &self.payload {
            ...
            Payload::Paint { list, glyphs, images, scenes } => {
                w.u8(4);
                put_list(&mut w, list);
                put_paint_rest(&mut w, glyphs, images.as_ref(), scenes);
            }
...
fn put_paint_rest(w: &mut W, glyphs: &[(u32, u32, u32, Vec<u8>)], images: Option<&(u32, u32, Vec<u8>)>, scenes: &[(Hash, crate::driver::SceneAsset)]) {
...
/// What a paint reply carries after its list: glyph rows, image rows, scene
/// assets.
type PaintRest = (Vec<(u32, u32, u32, Vec<u8>)>, Option<(u32, u32, Vec<u8>)>, Vec<(Hash, crate::driver::SceneAsset)>);
...
mod tests {
    ...
    fn replies_round_trip() {
        ...
        let all = vec![
            ...
            Payload::PaintAgain { glyphs: vec![(2, 1, 2, vec![0, 1])], images: None, scenes: vec![([5; 32], crate::driver::SceneAsset::Shader("fn main() {}".into()))] },
            Payload::Tick(true),
